var header = document.querySelector("header"); // <header> 태그
var part = document.querySelector("side").children; // <part> 태그
function clickNext() {
...
function Word() {
    var word = "Hadoop Guide";
    var word_split = word.split(""); // Hadoop Guide 를 배열로 바꿈
    ...
    word_split = word_split.concat(word.split("")); // 위의 문자를 쪼개서 배열에 추가
    var hello = document.createElement("hello"); // hello 클래스 생성
    ...
    var hello2 = document.createElement("hello"); // 또 다른 hello 클래스 생성
...
if (window.location.href === "http://localhost:3000/" || window.location.href === "https://juunini.github.io/hadoop/") {
    var start = document.createElement("start"); // start 태그 생성
