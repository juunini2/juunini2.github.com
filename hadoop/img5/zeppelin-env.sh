...
# export MASTER=spark://hadoop:7077                 		# Spark master url. eg. spark://master_addr:7077. Leave empty if you want to use local mode.
...
##
export SPARK_HOME=/home/hadoop/spark1                             # (required) When it is defined, load it instead of Zeppelin embedded Spark libraries
...
# export PYSPARK_PYTHON=${SPARK_HOME}/bin/pyspark          		# path to the python command. must be the same path on the driver(Zeppelin) and all workers.
...
export HBASE_HOME=/home/hadoop/hbase                    # (require) Under which HBase scripts and configuration should be
...
export ZEPPELIN_INTP_CLASSPATH_OVERRIDES=/home/hadoop/hive/conf